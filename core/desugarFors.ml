...
let results :  Types.row ->
  (Sugartypes.phrase list * CommonTypes.name list * Types.datatype list) -> Sugartypes.phrase =
  fun eff (es, xs, ts) ->
...
object (o : 'self_type)
  ...
  *)
  method qualifiers : Sugartypes.iterpatt list ->
    'self_type *
      (Sugartypes.phrase list * Sugartypes.Pattern.with_pos list * CommonTypes.name list *
         Types.datatype list) =
    fun qs ->
      let o, (es, ps, xs, ts) =
        List.fold_left
