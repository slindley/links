...
let compile_handle_cases
    : raw_env -> (raw_clause list * raw_clause list * (Ir.computation * pattern * Types.datatype) list * Desugartypes.handler_descriptor) -> Ir.computation -> Ir.computation =
  ...
  let handle =
    `Handle {
        ...
        ih_depth  =
          ...
          | Sugartypes.Shallow -> `Shallow
          | Sugartypes.Deep    -> `Deep params
      }
  in
